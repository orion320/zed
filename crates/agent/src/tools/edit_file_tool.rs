...
///
/// When `read_file` output includes line-number prefixes, each prefix is right-aligned
/// in a 6-character field followed by a single tab, then the line's actual content. When
/// you derive `old_text` or `new_text` from that output, strip this prefix and keep only
/// what comes after the tab, preserving the original indentation (tabs and spaces) exactly.
/// Never include any part of a line-number prefix in `old_text` or `new_text`.
